<!DOCTYPE HTML>

<html lang = "es">
 <head>
   <title>Venta por Referencia - Vendedor</title>

   <meta charset="iso-8859-1">
   <meta http-equiv="X-UA-Compatible" content="IE=edge">
   <meta name="viewport" content="width=device-width, initial-scale=1">
   <meta name="author" content="Grupo 6">

    <!-- Bootstrap core CSS -->
    <link href="../css/bootstrap.min.css" rel="stylesheet">

    <link rel="stylesheet" href="../css/principal.css" type="text/css" media="all" />
    <link rel="stylesheet" href="../css/ing_registro.css" type="text/css" media="all" />

    <!-- Script bootstrap -->
    <script src="../js/jquery.min.js"></script>
    <script src="../js/bootstrap.min.js"></script>

 </head>

 <body >
   
   <img id="bg" src="../images/fondo_2.png" alt="" />  
   

 <!--Menu-->
    <!--<div id="menu">-->
    <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
      <div class="container">
        <div class="navbar-header">
          <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#navbar" aria-expanded="false" aria-controls="navbar">
            <span class="sr-only">Toggle navigation</span>
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
          </button>
          <a class="navbar-brand">Ventas Por Referencia</a>
        </div>
        <div id="navbar" class="collapse navbar-collapse">
          <ul class="nav navbar-nav">
            <li><a href="perfil_vnd.html">Mi Perfil</a></li>
            <li><a href="contactar_vnd.html">Contactar</a></li>

            <li class="dropdown">
             <a class="dropdown-toggle" data-toggle="dropdown">
                Reportes
             <b class="caret"></b></a>

             <ul class="dropdown-menu">
               <li><a href="rpt_contacto_vnd.html">Contactados</a></li>
               <li><a href="rpt_prod_vnd.html">Prod. Vendidos</a></li>
             </ul>
           </li>

            <li><a href="../index.html">Cerrar Sesión</a></li>
          </ul>
        </div><!--/.nav-collapse -->
      </div>
    </nav>

  <div id="ctr_gen">
  <div class="container">
   <div class="starter-template">   
    <div id="Central">

    <!--Cabecera-->
      <div id="cabecera">
       <p id="Tit1">Ventas por Referencia</p>
        <img id="logo" src="../images/logo_espol.gif" alt=""/>
        <br>
        <p id="Tit2">Gana comisiones Referenciando a tus amigos <br> Tenemos lo que tú necesitas</p>
      </div>

      <!--Productos-->
      <div class="Buscador2">
        <h3>Lista de Productos</h3>
        <h4>Buscar</h4>
         <form method="post" action="bus_descrip">
            <select>
              <option value="Codigo">Código</option>
              <option value="Nombres">Descripción</option>
            </select>
            <input type="text" name="buscar" class="buscar" />
            <input type="submit" value="Buscar" class="boton">
         </form> 

        
            <table class="tblbuscar" >
               <caption> <strong> Resultado de la Busqueda</strong> </caption>
               <thead >
                  <tr >
                     <th class="tbl_cod" >Código</th>
                     <th  >Descripción</th>
                  </tr>
               </thead>
             </table> 

             <div class="d_tabla">
              <table class="tblbusdet" >
               <tbody>
                 <tr>
                   <td class="tbl_cod"><a href="#">1</a></td>
                   <td >RELOJ BLUETOOTH</td>
                 </tr>
                 <tr>
                   <td ><a href="#">2</a></td>
                   <td >SILLAS ERGONÓMICAS</td>
                 </tr>  
                 
               </tbody>
              </table>
             </div>
         </div>

         <!--Imnagen de Producto-->
      <div class="lateral">
          <div class="ingreso">
            <br>
           <img class="imgusr" src="../images/prod1.jpg" alt="" /> 
   
           <form method="post" action="carg_imagen">
              <br>
              <textarea rows="2" cols="20" class="det_nombres" name="nombres" readonly> RELOJ BLUETOOTH </textarea>
              <br> <br>     
           </form>

          </div>
      </div>
      <br>

      <!--Referidos-->
      <div class="Buscador2">
        <h3>Personas Referidas</h3>
        <h4>Ordenar por</h4>
         <form method="post" action="cons_refer">
            <select>
                <option value="Codigo">Código</option>
                <option value="Nombres">Nombres</option>
            </select>
            <input type="submit" value="Consultar" class="boton">
         </form> 

        
            <table class="tblbuscar" >
               <caption> <strong> Resultado de la Busqueda</strong> </caption>
               <thead >
                  <tr >
                     <th class="tbl_cod">Código</th>
                     <th class="tbl_nombre_rep3">Nombres</th>
                     <th >Cedula</th>
                  </tr>
               </thead>
             </table>

             <div class="d_tabla">
              <table class="tblbusdet" >
               <tbody>
                 <tr>
                   <td class="tbl_cod"><a href="#">1</a></td>
                   <td class="tbl_nombre_rep3">JOSE MIGUEL CARRASCO SANCHEZ</td>
                   <td >0918360951</td>
                 </tr>
                 <tr>
                   <td ><a href="#">2</a></td>
                   <td >MARIA CRISTINA GALARZA MARIN</td>
                   <td >0918360473</td>
                 </tr>

               </tbody>
              </table>
             </div>
         </div>

         <!--Imnagen de Referidos-->
      <div class="lateral">
          <div class="ingreso">
            <br>
           <img class="imgusr" src="../images/foto_usuario.jpg" alt="" /> 
   
           <form method="post" action="carg_imgper">
              <br>
              <textarea rows="2" cols="20" class="det_nombres" name="nombres" readonly> JOSE MIGUEL CARRASCO SANCHEZ </textarea>
              <br> <br>             
           </form>

          </div>
      </div>

      <br><br>
              <div id="form_contacto">
                <form method="post" action="proc_agendar">
                  <br>
                  <label>Observación</label>
                  <br>
                  <textarea rows="4" cols="25" id="dscprod" name="observacion" > </textarea>
                  <br><br>
                  <label>Contactado: </label>
                  <select>
                      <option value="S">Si</option>
                      <option value="N">No</option>
                  </select>
                  <label>&nbsp;&nbsp;Venta:</label>
                  <select>
                      <option value="V">Vendido</option>
                      <option value="N">No Interesado</option>
                  </select>
                  <br><br>
                  <label>Forma de Pago:</label>
                  <select>
                      <option value="D">Depósito </option>
                      <option value="T">Transferencia</option>
                  </select>
                  <label>&nbsp;&nbsp;Envio:</label>
                  <select>
                      <option value="S">Si</option>
                      <option value="N">No</option>
                  </select>
                  <br><br>
                  <label>Dirección Entrega</label>
                  <br>
                  <textarea rows="2" cols="25" id="dscprod2" name="entrega" > </textarea>
                  <br>
                  <input type="submit" name="grabar" value="Grabar" class="boton">
                  <br><br>
               </form> 
              </div>
              <div id="dat_contacto">
                <form method="post" action="carg_datos">
                    <label>Cedula</label>
                    <br>
                    <input type="text" class="ced" name="cedula" value="099999999" readonly/>
                    <br><br>
                    <label>Fecha de Contacto</label>
                    <br>
                    <input type="text" class="ced" name="fecha" value="28/12/2014" readonly/>
                    <br><br>
                    <label id="labtel1">Telefono 1</label>
                    <label id="labtel2">Telefono 2</label>
                    <br>
                    <input type="text" class="telf1" name="tel1" value="042496785" readonly/>
                    <input type="text" class="telf2" name="tel2" value="0978521456" readonly/>
                    <br>
                    <label>Email</label>
                    <br>
                    <input type="email" class="nomape" name="tel2" value="josecarr@gmail.com" readonly/>
                    <br><br><br>
                  </form>  
              </div>
              
      <!--Footer-->
      <div id="pie">
         <div>
            <a href="http://www.facebook.com"><img src="../images/facebook.jpg" alt=""/></a>            
            <a href="http://www.twitter.com"><img src="../images/twiter.jpg" alt=""/></a>            
            <a href="http://www.google.com"><img src="../images/googlemas.jpg" alt=""/></a>            
         </div>
         <p>Copyright © 2014 Ventitas. Todos los derechos reservados. Desarrollado por Grupo 6</p>
      </div>

    </div>
   </div>
  </div>

  </div>

 </body>

</html>